...
- Publishing is triggered by pushes to the `master` branch or manual workflow
  dispatch.
- The workflow uses Python Semantic Release to determine the next version from
  conventional commits and create the release commit and tag.
- `pyproject.toml` version updates are release-managed: do not bump
  `project.version` in regular feature or fix commits; Semantic Release writes
  the new version during the release commit.
