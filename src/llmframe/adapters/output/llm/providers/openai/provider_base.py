...
class OpenAIProviderBase:
    ...
    def _to_batch_submission(self, *, batch: object, request_count: int) -> LlmBatchSubmission:
        ...
        metadata = batch_object.metadata if hasattr(batch_object, "metadata") else None
    ...
    def _to_batch_status(self, *, batch: object) -> LlmBatchStatus:
        ...
        batch_object = cast("Any", batch)
        request_counts_value = getattr(batch_object, "request_counts", None)
        request_counts = None
        if request_counts_value is not None:
            request_counts = {
                "completed": cast("int", getattr(request_counts_value, "completed", 0)),
                "failed": cast("int", getattr(request_counts_value, "failed", 0)),
                "total": cast("int", getattr(request_counts_value, "total", 0)),
            }
        return LlmBatchStatus(
            ...
            metadata=cast("dict[str, JsonValue] | None", getattr(batch_object, "metadata", None)),
        )
    ...
    def _to_text_result_item(self, *, line: object) -> LlmBatchTextResultItem:
        ...
        response_body = getattr(line_object, "response_body", None)
        error = cast("str | None", getattr(line_object, "error", None))
