"""Input builders for provider-neutral LLM application use cases."""
...
from llmframe.application.dtos import LlmFileInputPart, LlmImageFileInputPart, LlmImageUrlInputPart, LlmTextInputPart
from llmframe.application.exceptions import StructuredLlmError
...
def require_existing_file(file_path: str | Path, *, kind_label: str) -> Path:
    """Return an existing regular file path or raise an application error."""
    path = Path(file_path)
    if not path.exists():
